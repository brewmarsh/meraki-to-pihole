...
def get_all_relevant_meraki_clients(dashboard: meraki.DashboardAPI, config: dict):
    ...
    logging.debug(
        f"Entering get_all_relevant_meraki_clients. Org ID: {org_id}. Specified Network IDs: {specified_network_ids if specified_network_ids else 'All'}. Client Timespan: {client_timespan}s."
    )
    all_clients_map = {}
    ...
    try:
        if not specified_network_ids:
            logging.info(
                f"No specific Meraki network IDs provided. Fetching all networks for organization ID: {org_id}."
            )
            # total_pages='all' should handle pagination for networks if there are many.
            organization_networks = dashboard.organizations.getOrganizationNetworks(
            ...
            logging.info(
                f"Specific Meraki network IDs provided: {specified_network_ids}. Validating and fetching details for these networks."
            )
            ...
            if not all_org_networks:  # Should not happen if IDs are specified, but defensive.
                logging.warning(
                    f"Could not fetch network list for organization {org_id} to validate specified IDs. Proceeding with specified IDs directly, but network names will be 'Unknown'."
                )
                networks_to_query_details = [
                    {"id": nid, "name": f"Unknown (ID: {nid})"} for nid in specified_network_ids
                ]
            else:
                ...
                for nid_spec in specified_network_ids:
                    if nid_spec in name_map:
                        ...
                        logging.warning(
                            f"Specified network ID {nid_spec} not found in organization {org_id}. It will be skipped."
                        )
                if not networks_to_query_details:
                    logging.warning(
                        f"None of the specified network IDs {specified_network_ids} were found/valid in organization {org_id}. No clients will be fetched."
                    )
                    return []
    ...
    if not networks_to_query_details:
        ...
        logging.info(
            f"Total networks to query: {len(networks_to_query_details)}. Network IDs: {[n['id'] for n in networks_to_query_details]}"
        )
    ...
    for network_idx, network_detail in enumerate(networks_to_query_details):
        ...
        logging.info(
            f"--- Processing network {network_idx + 1}/{len(networks_to_query_details)}: '{network_name}' (ID: {network_id}) ---"
        )
        ...
        try:
            ...
            try:
                ...
                appliances = dashboard.appliance.getNetworkAppliances(network_id)
                if appliances:
                    serial = appliances[0]["serial"]
                ...
                if e.status == 404 and "The requested URL was not found on the server." in str(
                    e.message
                ):  # More specific check
                    logging.info(
                        f"No DHCP subnet/VLAN configuration found via API for network {network_name} (ID: {network_id}) (Endpoint not found - likely not an MX network or no DHCP configured). Will rely on client.fixedIp if available."
                    )
                else:
                    logging.warning(
                        f"Could not retrieve DHCP subnet/VLANs info for network {network_name} (ID: {network_id}) to check reservations: {e}. Will rely on client.fixedIp if available."
                    )
            ...
            if appliance_dhcp_subnets:  # Could be a list of subnets/VLANs
                for subnet_info in appliance_dhcp_subnets:  # Iterate through each subnet/VLAN's DHCP settings
                    if subnet_info and "fixedIpAssignments" in subnet_info:
                        for mac, assignment_details in subnet_info[
                            "fixedIpAssignments"
                        ].items():  # It's a dict keyed by MAC
                            ...
                            if mac and assignment_details.get("ip"):
                                ...
                                logging.debug(
                                    f"Found configured DHCP reservation in network {network_id}: MAC {mac.lower()} -> IP {assignment_details['ip']} (Name: {assignment_details.get('name', 'N/A')})"
                                )
                logging.info(
                    f"Found {len(mac_to_reserved_ip_map)} DHCP fixed IP reservations in network {network_name} (ID: {network_id})."
                )

            # Fetch clients for the network
            clients_in_network = dashboard.networks.getNetworkClients(
                networkId=network_id, timespan=client_timespan, perPage=1000, total_pages="all"
            )

            if clients_in_network:
                logging.info(
                    f"SDK returned {len(clients_in_network)} clients for network '{network_name}' (ID: {network_id})."
                )
                ...
                for client in clients_in_network:
                    ...
                    if client_mac:
                        ...
                        if configured_fixed_ip:
                            logging.debug(
                                f"Client '{client_name}' (MAC: {client_mac.lower()}) found in DHCP reservations with IP {configured_fixed_ip}."
                            )
                    ...
                    if not configured_fixed_ip and client.get("fixedIp"):
                        ...
                        logging.debug(
                            f"Client '{client_name}' (MAC: {client_mac.lower()}) not in DHCP reservations map, but client object has fixedIp: {configured_fixed_ip_from_client_obj}. Using this."
                        )
                    ...
                    if (
                        not configured_fixed_ip and not mac_to_reserved_ip_map
                    ):  # If no DHCP reservations were found for the network at all
                        configured_fixed_ip = client.get("fixedIp")  # Fallback to the client's reported fixedIp
                        if configured_fixed_ip:
                            logging.debug(
                                f"No DHCP reservations found for network {network_name}. Client '{client_name}' (MAC: {client_mac.lower()}) has fixedIp attribute: {configured_fixed_ip}. Using this as potential configured IP."
                            )
                    ...
                    if client_name and current_ip and client_id:
                        if configured_fixed_ip and configured_fixed_ip == current_ip:
                            ...
                            logging.info(
                                f"Relevant client: '{client_name}' (IP: {current_ip}, Meraki ID: {client_id}) in network '{network_name}'. Configured Fixed IP ({configured_fixed_ip}) matches current IP. Will be processed."
                            )
                        elif configured_fixed_ip:
                            logging.debug(
                                f"Client '{client_name}' (ID: {client_id}) in network '{network_name}' has configured Fixed IP ({configured_fixed_ip}) but current IP ({current_ip}) differs. Skipping."
                            )
                        else:
                            logging.debug(
                                f"Client '{client_name}' (ID: {client_id}) in network '{network_name}' does not have a usable Fixed IP assignment (Configured: '{configured_fixed_ip}', MAC in map: {bool(client_mac and client_mac.lower() in mac_to_reserved_ip_map)}). Skipping."
                            )
                    else:
                        logging.debug(
                            f"Skipping client (Meraki ID: {client.get('id')}, MAC: {client.get('mac')}) in network '{network_name}' due to missing name, current IP, or client ID."
                        )
            else:
            ...
            logging.error(
                f"An unexpected error occurred while processing network {network_name} (ID: {network_id}): {e}",
                exc_info=True,
            )

        logging.info(
            f"--- Finished processing network {network_idx + 1}/{len(networks_to_query_details)}: {network_name} (ID: {network_id}) ---"
        )
