...
class TestMerakiPiholeSync(unittest.TestCase):
    ...
    def test_main_success_flow(self, mock_dashboard_api, mock_add_or_update, mock_get_dns, mock_get_clients, mock_auth, mock_load_config):
        # Arrange
        ...
        # Assert
        mock_auth.assert_called_once_with("http://fake-pihole.local", "fake_pihole_key")
        mock_get_clients.assert_called_once()
        mock_get_dns.assert_called_once_with("http://fake-pihole.local", "fake_sid", "fake_csrf")
        mock_add_or_update.assert_called_once_with(
            "http://fake-pihole.local",
            "fake_sid",
            "fake_csrf",
            "test-client-1.lan",
            "192.168.1.10",
