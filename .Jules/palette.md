...
## 2026-07-23 - Focusable Scrollable Regions
**Learning:** Visually scrollable regions (like `<pre>` elements with `overflow: scroll`) cannot be easily scrolled by keyboard users unless the element itself can receive focus. Without focus, arrow keys might scroll the entire page instead of the inner content. Adding `tabindex="0"` solves this by putting the element in the tab order. Additionally, `aria-label` provides necessary context for screen readers when the element gains focus, and `:focus-visible` styling ensures sighted keyboard users know where their focus currently is.
**Action:** Always add `tabindex="0"`, descriptive `aria-label`s, and clear `:focus-visible` CSS outlines to any visually scrollable container (like log boxes, code snippets, or custom scrolling areas) to ensure they are fully navigable and understandable for keyboard and screen reader users.
