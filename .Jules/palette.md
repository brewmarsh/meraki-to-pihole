...
## 2024-07-18 - Safe Accessible Modal Overlays
**Learning:** Adding standard UX dismissal patterns (like Escape key and backdrop clicks) to conditionally rendered modals (via Jinja2) can easily throw `TypeError` if event listeners are attached blindly. Furthermore, adding standard `role="dialog"`, `aria-modal="true"`, and linking a title via `aria-labelledby` is essential for screen readers to interpret custom overlay divs correctly.
**Action:** When implementing custom modal overlays, ensure standard `Escape` key and backdrop click listeners are included for usability. Always wrap these event listeners in an existence check (e.g., `if (document.getElementById('modal'))`) when the element is conditionally rendered by the backend to prevent script failure.
