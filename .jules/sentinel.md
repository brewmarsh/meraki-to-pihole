...
## 2026-07-25 - Fix Infinite Recursion DoS in Automatic Retries
**Vulnerability:** The application blindly retried 403 HTTPError responses from the Pi-hole API without any retry limit, leading to an infinite recursion and Denial of Service (DoS) if the API consistently returned an error.
**Learning:** Unbounded recursive retries on external API errors can exhaust the call stack or consume threads infinitely, crashing the application.
**Prevention:** Always use a bounded depth mechanism (like a `_retry=True` flag or a retry counter) when implementing automatic error retries for external API calls.
