...
## 2026-08-04 - Fix Infinite Recursion DoS in PiholeClient
**Vulnerability:** The `PiholeClient._api_request` function unconditionally retried authentication and recursively called itself upon receiving a 403 Forbidden HTTP error. If the API consistently returned 403, this caused infinite recursion resulting in a stack overflow and a Denial of Service (DoS).
**Learning:** Automatic error retries for external APIs using recursive function calls are dangerous if unbounded.
**Prevention:** Always use a bounded depth mechanism (like a `_retry=True` boolean flag or an integer retry counter) when implementing recursive API retry logic to prevent infinite recursion on consistent errors.
