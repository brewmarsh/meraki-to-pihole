...
## 2023-10-27 - Bounded Error Retries

**Vulnerability:** Found an infinite recursion (DoS) vulnerability in `app/clients/pihole_client.py` where a repeated 403 HTTP response would trigger a recursive call to `_api_request` to re-authenticate without a base case to exit.
**Learning:** External APIs that repeatedly fail (e.g., due to an invalidated API token or bad permissions) can trigger infinite loops in naive recursive error handlers, leading to stack overflow and Denial of Service (DoS) in the application worker threads.
**Prevention:** When implementing automatic error retries for external API calls, always use a bounded depth mechanism (like a `_retry=True` flag or a retry counter) to prevent infinite recursion and DoS crashes if the API consistently returns an error.
