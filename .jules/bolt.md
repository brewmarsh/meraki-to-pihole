...
## 2024-05-24 - Server-Sent Events (SSE) Duplicate API Calls Bottleneck
**Learning:** The `/stream` SSE endpoint calls `get_mappings_data` inside a `while True:` loop every few seconds. Since `get_mappings_data` originally performed API requests to Meraki and Pi-hole directly, every connected client caused N+1 API calls, scaling the backend load linearly with the number of open browser tabs (O(C*API)).
**Action:** When working with SSE streams that push data generated by a backend sync task, always cache the results of the background job (e.g. in `cache.json` or memory) and have the SSE clients read from the cache instead of independently hitting external APIs.
