...
## 2025-10-27 - Fallbacks for background-synced files
**Learning:** When optimizing live API endpoints by reading from a background-synced file like `cache.json`, there is a risk of reading the file exactly when the background thread is overwriting it, causing a `JSONDecodeError`.
**Action:** Always wrap file reads of background-synced JSON files in a `try...except json.JSONDecodeError` block and provide a safe fallback (like returning cached memory or old API logic) to prevent runtime crashes.

## 2025-10-27 - UI payload schema mismatch from caches
**Learning:** Background scripts (like `sync_logic.py`) might optimize their `cache.json` output by dropping heavy lists for simple integer counts. The frontend UI might rely on those heavy lists.
**Action:** When replacing an API data source with a background cache, carefully inspect the structure generated by the background script against the structure expected by the frontend. Manually recompute missing lists (like running `_map_devices()`) if necessary.
